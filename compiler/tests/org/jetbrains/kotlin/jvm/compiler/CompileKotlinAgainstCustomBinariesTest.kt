...
class CompileKotlinAgainstCustomBinariesTest : AbstractKotlinCompilerIntegrationTest() {
    ...
    fun testStrictMetadataVersionSemanticsSameVersion() {
        val library = compileLibrary("library", additionalOptions = listOf("-Xstrict-metadata-version-semantics"))
        compileKotlin("source.kt", tmpdir, listOf(library))
    }

    fun testStrictMetadataVersionSemanticsOldVersion() {
        val library = compileLibrary(
            "library", additionalOptions = listOf("-Xstrict-metadata-version-semantics", "-Xmetadata-version=1.4.0")
        )
        compileKotlin("source.kt", tmpdir, listOf(library))
    }
